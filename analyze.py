...
class SQL_Query(ChatGPT_Handler):
    ...
    def insert_sql_query(self, query, limit=10000):  
        if self.db_path is not None:  
            engine = create_engine(f'sqlite:///{self.db_path}')  
        else:  
            connecting_string = f"Driver={{ODBC Driver 17 for SQL Server}};Server=tcp:{self.dbserver},1433;Database={self.database};Uid={self.db_user};Pwd={self.db_password}"
        ...
        # session.close()  
        return "Your insert was successful to " + self.database + ". Please check the database to verify." 
...
class AnalyzeGPT(ChatGPT_Handler):
    ...
    def insert_run(self, question: str, show_code,show_prompt,st) -> any:
        st.write(f"Question: {question}")
        def insert_sql(query):
            return self.sql_query_tool.insert_sql_query(query)
        max_steps = 15
        count =1
